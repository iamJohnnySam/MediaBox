...
def grapher_category_dictionary(graph_dict, x_column, cat_column, data_column, x_name, y_name, chart_title):
    ...
    for cat in data.keys():
        plt.bar(list(data[cat].keys()), list(data[cat].values()), color=colors[color_val], label=cat)

        color_val = color_val + 1
        if color_val == len(colors):
            color_val = 0

    plt.plot(plt.bar(list(total_data.keys()), list(total_data.values()), color=colors[color_val], label="Total"))
