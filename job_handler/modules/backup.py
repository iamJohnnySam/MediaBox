...
class BackUp(Module):
    ...
    def __init__(self, job: Job):
        ...
        logger.log(self.job.job_id, "Object Created")

    def run_backup(self):
        logger.log(self.job.job_id, "Backup Started")
        self.cp_files()
        self.mv_files()
        self.cp_folders()
        self.mv_folders()
        self.cp_databases()
        logger.log(self.job.job_id, "Backup Ended")
    ...
    def cp_files(self):
        for file in self.copy_files:
            ...
            if not os.path.exists(os.path.dirname(destination)):
                ...
                logger.log(self.job.job_id, f"Directory created > {os.path.dirname(destination)}")

            shutil.copy(file, destination)
            logger.log(self.job.job_id, f"Copied {file} -> {destination}")
    ...
    def mv_files(self):
        for file in self.move_files:
            ...
            try:
                ...
                logger.log(self.job.job_id, f"Moved {file} -> {destination}")
    ...
    def cp_folders(self):
        for folder in self.copy_folders:
            ...
            logger.log(self.job.job_id, f"Copied {folder} -> {destination}")
    ...
    def mv_folders(self):
        for folder in self.move_folders:
            ...
            for file_path in allfiles:
                ...
                logger.log(self.job.job_id, f"Moved {file_path} -> {dst_path}")
    ...
    def _backup_database(self, database, backup_file_path):
        ...
        os.system(gzip_cmd)
        logger.log(self.job.job_id, f'BackUp {database} database > {backup_file_path}.')
